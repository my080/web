...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      getView: function(){
        this.tableData = gbqModel().getView('qtxm');
      ...
      doPaste: function(){
        ...
        if(this.clipboard.length>0&&record){
          var vueObj = this;            
          gbqModel().pasteItem('qtxm', this.clipboard, this.currentRowNo(), this.isCut, function(){
            vueObj.getView();          
          });
        }
      },
      doMoveUp: function(){
        ...
        if(row){
          gbqModel().moveUp('qtxm', vueObj.currentRowNo(), true, function(data, dst){
            vueObj.getView();
      ...
      doMoveDown: function(){
        ...
        var row = vueObj.currentRecord();
        if(row){
          gbqModel().moveDown('qtxm', vueObj.currentRowNo(), true, function(data, dst){
            vueObj.getView();
            vueObj.setCurrentRow(row);          
          });  
        }        
      ...
      doDelete: function () {   //删除表格数据
        ...
        if(selected.length>0){
          vueObj.$confirm('是否确认删除？', '提示', {
            ...
            gbqModel().deleteItems('qtxm', selected, function(){              
              vueObj.getView();              
            });
          });
        }        
        ...
        if(command=='title'){
          ...
          if(record){
            if(record.type=='分部'){
              ...
              gbqModel().insertTitle('qtxm', newTitle, insertIdx, function(){
                vueObj.getView();
            ...
            newTitle.level = 0;
            newTitle.type = '分部';
            gbqModel().insertTitle('qtxm', newTitle, -1, function(){
              vueObj.getView();
          ...
          if(record&&record.type=='分部'&&record.itemType=='子分部'){
            ...
            gbqModel().insertTitle('qtxm', newTitle, rowno + 1, function(){
              vueObj.getView();
          ...
          if(record) {            
            if(record.type=='清单') { // 插入同级项
              ...
              gbqModel().insertQtxmItem(insertIdx, record.pid, function(){
                vueObj.getView();
                vueObj.setCurrentRow(vueObj.tableData[insertIdx]);
              });  
            } else {
                // 自动展开当前记录
              this.$refs.table.$emit('set-row-collapse', record, false);
              gbqModel().insertQtxmItem(rowno + 1, record.id, function(){
                vueObj.getView();
                vueObj.setCurrentRow(vueObj.tableData[rowno + 1]);
              });  
            }
            ...
            if(vueObj.tableData.length==0){
              gbqModel().insertQtxmItem(-1, null, function(){
                vueObj.getView();               
              }); 
            } else {
              this.$message({
      ...
      insertBQItem: function(bqItem, lmmDetail, rateDict, rateDetail){
        ...
        gbqModel().insertQtxmItemFromDB(bqItem, lmmDetail, rateDict, rateDetail, recno, pid, function(){
          vueObj.getView();
